...
try:
    from gtd_manager.models import (
        GTDItem, Action, Project, Context,
        Organization, Team, Stakeholder, GTDItemStakeholder,
        GTDStatus, RACIRole, OrganizationType
    )
except ImportError:
...
class TestProjectEntity:
    ...
    def test_project_creation_inherits_from_gtd_item(self):
        ...
        assert project.success_criteria == "Product is live and first customer onboarded"
...
class TestContextEntity:
    ...
    def test_context_creation_with_validation(self):
        ...
        context = Context(
            name="@computer",
            description="Tasks requiring computer work"
        )
...
class TestOrganizationEntity:
    ...
    def test_organization_type_validation(self):
        ...
        partner_org = Organization(name="Tech Partner Inc", type=OrganizationType.PARTNER)
...
class TestGTDItemStakeholderJunction:
    ...
    def test_exactly_one_accountable_constraint(self):
        ...
        with pytest.raises(ValueError, match="Only one Accountable allowed per GTD item"):
            GTDItemStakeholder(
...
class TestIntegrationScenarios:
    ...
    def test_project_with_multiple_actions_and_stakeholders(self):
        ...
        designer = Stakeholder(name="Carol Designer", email="carol@amazon.com", organization_id="org123")
        developer = Stakeholder(name="Dave Developer", email="dave@amazon.com", organization_id="org123")
        pm = Stakeholder(name="Eve PM", email="eve@amazon.com", organization_id="org123")
        
        # Project-level stakeholder (PM is accountable for overall project)
        project_rel = GTDItemStakeholder(
            gtd_item_id=project.id,
            stakeholder_id=pm.id,
            raci_role=RACIRole.ACCOUNTABLE
        )
