...
class Project(GTDItem):
    ...
    def __setattr__(self, name: str, value) -> None:
        ...
        if (name == 'status' and value == GTDStatus.ORGANIZED and 
            hasattr(self, '_initialized') and self._initialized):
            if not getattr(self, 'success_criteria', None):
    ...
    def get_completed_actions(self) -> List['Action']:
        ...
        """
        return [action for action in self.actions if action.status == GTDStatus.COMPLETE]
