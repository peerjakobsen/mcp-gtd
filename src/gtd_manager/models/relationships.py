...
class GTDItemStakeholder:
    ...
    def __post_init__(self):
        ...
        if self.raci_role == RACIRole.ACCOUNTABLE:
            ...
            existing_accountable = [
                rel
                for rel in _raci_relationships
                if (
                    rel.gtd_item_id == self.gtd_item_id
                    and rel.raci_role == RACIRole.ACCOUNTABLE
                    and rel.stakeholder_id != self.stakeholder_id
                )
            ]
    ...
    def get_relationships_for_item(cls, gtd_item_id: str) -> list["GTDItemStakeholder"]:
        ...
        """
        return [rel for rel in _raci_relationships if rel.gtd_item_id == gtd_item_id]

    @classmethod
    def get_relationships_for_stakeholder(
        cls, stakeholder_id: str
        ...
        """
        return [
            rel for rel in _raci_relationships if rel.stakeholder_id == stakeholder_id
        ]

    @classmethod
    def get_stakeholders_by_role(
        cls, gtd_item_id: str, role: RACIRole
    ) -> list["GTDItemStakeholder"]:
        """
